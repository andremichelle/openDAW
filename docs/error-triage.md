...
| fixed | `Storage not available` | Both crashes fire on "Save As..." (Mac Safari/Firefox). OPFS (`getOpfsRoot` → `navigator.storage.getDirectory()`, OpfsWorker.ts:131) rejected AFTER a successful boot probe (`boot.ts:148` runs the identical call and gates startup), so storage went unavailable post-boot (flakiness / eviction). `ProjectProfileService.save`/`saveAs` were the only storage flows not wrapped in `tryCatch`, so the rejection escaped to the global handler as an uncaught crash | `92936c29a` (wrap both writes; retryable "Could not save project (storage temporarily unavailable)" notice, profile + unsaved changes preserved) | live ids 1055, 1024 |
| environmental | `Failed to load because no supported source was found.` | An `HTMLMediaElement` with no decodable source — environmental, not a logic bug | (no fix) | live id 1022 |
