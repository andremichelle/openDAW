...
export class ProjectMigration {
    static async migrate(env: ProjectEnv, {boxGraph, mandatoryBoxes}: ProjectSkeleton) {
        ...
        boxGraph.boxes().slice().forEach(box => box.accept<BoxVisitor>({
            ...
            visitValueEventCollectionBox: (collectionBox: ValueEventCollectionBox): void => {
                // Fix duplicate ValueEvents: events at the same position must have unique indices
                // Rule: Only two events can share the same position (index 0 and 1), all in-between are deleted
