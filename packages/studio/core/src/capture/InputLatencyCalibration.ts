...
import {dbToGain, generateMls, LatencyCalibrationProtocol} from "@opendaw/lib-dsp"
 ...
 * See @opendaw/lib-dsp latency-calibration for the probe and the analysis.
 */
export namespace InputLatencyCalibration {
    ...
    export const emptyResult = (verdict: Verdict, sampleRate: number, scheduledBursts: int, now: number): Result => ({
    ...
    export const measure = async (context: AudioContext, source: AudioNode, output: AudioNode,
        ...
        if (context.state !== "running") {return emptyResult("context-not-running", sampleRate, burstCount, now())}
        ...
        try {
            ...
            capture.stop().catch(() => {})
            gainNode.disconnect()
            return emptyResult("context-not-running", sampleRate, burstCount, now())
        }
        ...
        if (analysis.identifiedBursts === 0) {return emptyResult("no-signal", sampleRate, burstCount, now())}
