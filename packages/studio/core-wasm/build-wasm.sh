...
DEVICE_CRATES="device-korpus device-cubed device-autotune device-revamp device-pitch device-arpeggio device-zeitgeist device-tidal device-vaporisateur device-neon device-nano device-delay device-playfield-sample device-gate device-werkstatt device-apparat device-spielwerk device-waveshaper device-crusher device-fold device-stereo-tool device-velocity device-maximizer device-compressor device-reverb device-dattorro-reverb device-convolver device-soundfont device-vocoder device-neural-amp"
