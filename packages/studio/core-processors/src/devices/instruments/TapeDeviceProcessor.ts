...
export class TapeDeviceProcessor extends AbstractProcessor implements DeviceProcessor, AudioGenerator {
    ...
    #updateOrCreatePitchVoice(lane: Lane, sourceUuid: UUID.Bytes, data: AudioData,
        ...
        if (existing === null) {
            ...
            // Voice is mid-internal-end-of-file-fade. Don't evict to fadingVoices —
            // that path uses unit gain and would drop the region's fade buffer for
            // the remaining ~20 ms, producing an audible pop at the region end.
