import css from "./EntryPeakMeter.sass?inline"
...
const className = Html.adoptStyleSheet(css, "EntryPeakMeter")
...
export const EntryPeakMeter = ({lifecycle, receiver, address}: Construct) => {
    ...
    lifecycle.own(Html.watchResize(element, () => {
        ...
        const bars: Array<SVGRectElement> = Arrays.create(channelIndex => (
            <rect x={barPadding + (barWidth + barPadding) * channelIndex} y={barPadding}
                  width={barWidth} height={0} fill={`url(#${gradientID})`}/>
        ), numChannels)
        element.appendChild(
            <svg viewBox={`0 0 ${width} ${height}`} width={width} height={height}>
                <defs>
                    <linearGradient id={gradientID} x1="0" x2="0" y2={barPadding} y1={innerHeight}
