...
export const OutSection = (construct: SectionConstruct) => {
    ...
    return (
        <div className={`${className} rows-3`}>
            ...
            {display([1, 4], [1, 5], <TubularFilterDisplay lifecycle={lifecycle} cutoff={cutoff} resonance={resonance}/>)}
            {labelRadio(lifecycle, "Play-Mode", EditWrapper.forAutomatableParameter(editing, voicingMode), ["MONO", "POLY"])}
            {labelRadio(lifecycle, "Engine", EditWrapper.forValue(editing, adapter.box.engine), Tubular.Engines, 2)}
