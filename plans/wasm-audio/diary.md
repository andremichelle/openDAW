...
- Master / output-unit effects: the engine now wires THE output unit's own audio-effect chain between the summing bus and the master strip, binding each device's parameters like an instrument unit's. The Multiple Plugins page puts a master Tidal tremolo there, so it pumps the whole mix (both parts together) rather than a single unit. Done.
