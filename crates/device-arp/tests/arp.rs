...
fn note(kind: u32, id: u32, pitch: u32) -> EventRecord {
    EventRecord {offset: 0, kind, id, pitch, velocity: 0.8, cent: 0.0}
}

fn blank() -> EventRecord {
    EventRecord {offset: 0, kind: 0, id: 0, pitch: 0, velocity: 0.0, cent: 0.0}
}
...
fn arpeggiates_a_held_chord_across_blocks_with_no_new_input() {
    ...
    let first = step(&mut state, 0.0, 240.0, PLAYING, |pulse| pulse as u32, &mut out);
    ...
    let second = step(&mut state, 240.0, 480.0, PLAYING, |pulse| pulse as u32, &mut out);
...
fn stops_emitting_when_the_chord_is_released() {
    ...
    let mut out = [blank(); 64];
    step(&mut state, 0.0, 240.0, PLAYING, |pulse| pulse as u32, &mut out);
    // release the whole chord (note-offs from upstream)
    ingest(&mut state, &[note(EVENT_NOTE_OFF, 1, 60), note(EVENT_NOTE_OFF, 2, 64)]);
    let count = step(&mut state, 240.0, 480.0, PLAYING, |pulse| pulse as u32, &mut out);
...
fn a_transport_jump_releases_everything_held() {
    ...
    ingest(&mut state, &[note(EVENT_NOTE_ON, 1, 60), note(EVENT_NOTE_ON, 2, 64)]);
    let mut out = [blank(); 64];
    step(&mut state, 0.0, 480.0, PLAYING, |pulse| pulse as u32, &mut out); // two steps -> two notes ringing
    let count = step(&mut state, 0.0, 10.0, PLAYING | BLOCK_FLAG_DISCONTINUOUS, |pulse| pulse as u32, &mut out);
