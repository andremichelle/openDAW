//! swarm, a polyphonic sampler instrument, as a runtime-loadable device: a faithful port of the (retired)
//! TS `SwarmDeviceProcessor`. It plays ONE loaded sample (its `file` pointer) per note, each voice a
...
// The Swarm box's field-key paths (the stable schema keys): volume `[10]` (decibel), octave `[11]`,
...
impl Instrument for Swarm {
    type State = SwarmState;

    fn init(state: &mut SwarmState, sample_rate: f32) {
    ...
    fn handle_event(state: &mut SwarmState, event: &EventRecord) {
    ...
    fn process_audio(state: &mut SwarmState, output: [&mut [f32]; 2], _block: &Block) {
    ...
    fn parameter_changed(state: &mut SwarmState, id: u32, value: ParamValue) {
        if id == state.gain_id {
    ...
    fn sample_changed(state: &mut SwarmState, id: u32, sample: Option<u32>) {
        if id == state.sample_id {
            state.sample = sample;
        }
    }

    fn reset(state: &mut SwarmState) {
        for voice in state.voices.iter_mut() {
            voice.force_stop();
        }
    }
...
fn publish_positions(state: &mut SwarmState, _frame_count: f32) {
...
pub fn render(state: &mut SwarmState, events: &[EventRecord], out_left: &mut [f32], out_right: &mut [f32], sample_rate: f32) {
    state.sample_rate = sample_rate;
    ...
    abi::dispatch_range::<Swarm>(state, [&mut *out_left, &mut *out_right], events, &block);
    Swarm::finish(state, [out_left, out_right]);
}
...
pub extern "C" fn state_size(_sample_rate: f32) -> u32 {
    core::mem::size_of::<SwarmState>() as u32
}

#[no_mangle]
pub extern "C" fn process(desc_ptr: u32) {
    let ports = unsafe { Ports::<SwarmState>::from_descriptor(desc_ptr) };
    abi::render_instrument::<Swarm>(ports);
}
...
pub extern "C" fn init(state_ptr: u32, sample_rate: f32) {
    unsafe { abi::with_state(state_ptr, |state| <Swarm as Instrument>::init(state, sample_rate)) }
}
...
pub extern "C" fn parameter_changed(state_ptr: u32, id: u32, kind: u32, value: f32, modulation: f32) {
    unsafe { abi::with_state(state_ptr, |state| <Swarm as Instrument>::parameter_changed(state, id, ParamValue::from_wire(kind, value, modulation))) }
}
...
pub extern "C" fn sample_changed(state_ptr: u32, id: u32, handle: u32, present: u32) {
    ...
    unsafe { abi::with_state(state_ptr, |state| <Swarm as Instrument>::sample_changed(state, id, sample)) }
}

/// Transport STOP: drop every voice so playback starts silent.
#[no_mangle]
pub extern "C" fn reset(state_ptr: u32) {
    unsafe { abi::with_state(state_ptr, |state| <Swarm as Instrument>::reset(state)) }
}
...
mod tests {
    ...
    fn silent_without_a_resident_sample() {
        let mut state: SwarmState = unsafe { core::mem::zeroed() };
        state.sample = Some(1);
